...
ClassImp(TA2TestEGPhysics)

//_____________________________________________________________________________________
TA2TestEGPhysics::TA2TestEGPhysics( const char* name, TA2Analysis* analysis ) : TA2Physics( name, analysis )
{
...
//_____________________________________________________________________________________
TA2TestEGPhysics::~TA2TestEGPhysics()
{
...
//_____________________________________________________________________________________
void TA2TestEGPhysics::SetConfig(Char_t* line, Int_t key)
{
...
//_____________________________________________________________________________________
void TA2TestEGPhysics::PostInit()
{
...
//   fCB = (TA2CB*)((TA2Analysis*)fParent)->GetChild("CB");
  ...
  fMggCBTAPS = new Double_t[fNggMax+1];
  fMggTAPS = new Double_t[fNggMax+1];
...
//_____________________________________________________________________________________
void TA2TestEGPhysics::LoadVariable()
{
  ...
  TA2DataManager::LoadVariable("MggCB",    	 	fMggCB,		         EDMultiX);
  TA2DataManager::LoadVariable("MggBestCB",    	 	&fMggBestCB,	         EDSingleX);
  TA2DataManager::LoadVariable("HasPi0CB",    	 	&fHasPi0CB,	         EISingleX);
  TA2DataManager::LoadVariable("HasPi0CBPol10",     	&fHasPi0CBPol10,         EISingleX);
  TA2DataManager::LoadVariable("HasPi0CBPol20",     	&fHasPi0CBPol20,         EISingleX);
  //
  TA2DataManager::LoadVariable("NggCBTAPS",    	 	&fNggCBTAPS,	         EISingleX);
  TA2DataManager::LoadVariable("MggCBTAPS",    	 	fMggCBTAPS,	         EDMultiX);
  TA2DataManager::LoadVariable("MggBestCBTAPS",     	&fMggBestCBTAPS,         EDSingleX);
  TA2DataManager::LoadVariable("HasPi0CBTAPS",     	&fHasPi0CBTAPS,	         EISingleX);
  TA2DataManager::LoadVariable("HasPi0CBTAPSPol10",    	&fHasPi0CBTAPSPol10,     EISingleX);
  TA2DataManager::LoadVariable("HasPi0CBTAPSPol20",    	&fHasPi0CBTAPSPol20,     EISingleX);
  //
  TA2DataManager::LoadVariable("NggTAPS",    	 	&fNggTAPS,	         EISingleX);
  TA2DataManager::LoadVariable("MggTAPS",    	 	fMggTAPS,	         EDMultiX);
  TA2DataManager::LoadVariable("MggBestTAPS",     	&fMggBestTAPS,         EDSingleX);
  TA2DataManager::LoadVariable("HasPi0TAPS",     	&fHasPi0TAPS,	         EISingleX);
  TA2DataManager::LoadVariable("HasPi0TAPSPol10",    	&fHasPi0TAPSPol10,     EISingleX);
  TA2DataManager::LoadVariable("HasPi0TAPSPol20",    	&fHasPi0TAPSPol20,     EISingleX);
  //
  TA2DataManager::LoadVariable("HasPi0",     		&fHasPi0,		 EISingleX);
  TA2DataManager::LoadVariable("HasPi0Pol10",    	&fHasPi0Pol10,		 EISingleX);
...
//_____________________________________________________________________________________
void TA2TestEGPhysics::Reconstruct()
{
  ...
  // Cherenkov TDC
  if(!fIsSim) {
    fADC1411M0 = fMulti[2001]->GetHit(0);
    if(fCheckCher) fHasCherCoinc = HasCherCoinc(fADC1411M0);
  }
  
  // Beam polarization
  fBeamPol = (Int_t)fADC[7]&kBeamPolConst;
  if(fBeamPol==5 || fBeamPol == 1 ) fBeamPol10 = 1;
  if(fBeamPol==8) fBeamPol20 = 1;
  ...
  // CB bgd
  TA2Particle *pCB = NULL;
  if(fCB) pCB = fCB->GetParticles();
  ...
  // TAPS bgd
  TA2Particle *pTAPS;
  if(fTAPS) pTAPS = fTAPS->GetParticles();
...
//_____________________________________________________________________________________
void TA2TestEGPhysics::ReconstructPi0()
{
  // pi0 test reconstruction
  
  if(!fCB) return;
  
  Int_t fNtracksCB = fCB->GetNtracks();
  const TA2CentralTrack *tracks = fCB->GetTracks();
  Double_t min = 1000.;
  for (Int_t i=0; i<fNtracksCB-1; ++i)
  {
    if (!tracks[i].IsNeutral()) continue;
//     if (tracks[i].GetEclNaI() < 30.) continue;
    TLorentzVector g1 = TA2Math::LorentzVector(tracks[i].GetDirCos().Unit(),tracks[i].GetEclNaI(),0.);
    for (Int_t j=i+1; j<fNtracksCB; ++j)
    {
      if (!tracks[j].IsNeutral()) continue;
//       if (tracks[j].GetEclNaI() < 30.) continue;
//       if ( (tracks[i].GetEclNaI() + tracks[j].GetEclNaI()) < 100. ) continue;
      TLorentzVector g2 = TA2Math::LorentzVector(tracks[j].GetDirCos().Unit(),tracks[j].GetEclNaI(),0.);
      fMggCB[fNggCB] = (g1 + g2).Mag();
      Double_t diff = TMath::Abs(fMggCB[fNggCB]-134.97666);
      if (diff <= min)
      {
	fMggBestCB = fMggCB[fNggCB];
	min = diff;
      }
      ++fNggCB;
      if (fNggCB == fNggMax) break;
    }
  }
  
  // >=1 pi0 in CB
  fHasPi0CB = fMggBestCB >= 80. && fMggBestCB <= 200.;
  fHasPi0CBPol10 = fHasPi0CB && fBeamPol10;
  fHasPi0CBPol20 = fHasPi0CB && fBeamPol20;
  
  // Charged
  for (Int_t i=0; i<fNtracksCB; ++i)
  {
  ...
  // pi0 CB-TAPS
  if (!fTAPS) return;
  Int_t fNtracksTAPS = fTAPS->GetNparticle();
  TA2Particle *pTAPS = fTAPS->GetParticles();
  min = 1000.;
  for (Int_t i=0; i<fNtracksCB; ++i)
  {
    if (!tracks[i].IsNeutral()) continue;
    TLorentzVector gCB = TA2Math::LorentzVector(tracks[i].GetDirCos().Unit(),tracks[i].GetEclNaI(),0.);
    for (Int_t j=0; j<fNtracksTAPS; ++j)
    {
      TLorentzVector gTAPS = TA2Math::LorentzVector(pTAPS[j].GetDirection(),pTAPS[j].GetKinetic(),0.);
      fMggCBTAPS[fNggCBTAPS] = (gCB + gTAPS).Mag();
      Double_t diff = TMath::Abs(fMggCBTAPS[fNggCBTAPS]-134.97666);
      if (diff <= min)
      {
	fMggBestCBTAPS = fMggCBTAPS[fNggCBTAPS];
	min = diff;
      }
      ++fNggCBTAPS;
      if (fNggCBTAPS == fNggMax) break;
    }
  }
  
  // >=1 pi0 in CB-TAPS
  fHasPi0CBTAPS = fMggBestCBTAPS >= 80. && fMggBestCBTAPS <= 200.;
  fHasPi0CBTAPSPol10 = fHasPi0CBTAPS && fBeamPol10;
  fHasPi0CBTAPSPol20 = fHasPi0CBTAPS && fBeamPol20;
  
  // pi0 in TAPS
  min = 1000.;
  for (Int_t i=0; i<fNtracksTAPS-1; ++i)
  {
    TLorentzVector g1 = TA2Math::LorentzVector(pTAPS[i].GetDirection(),pTAPS[i].GetKinetic(),0.);
    for (Int_t j=i+1; j<fNtracksTAPS; ++j)
    {
      TLorentzVector g2 = TA2Math::LorentzVector(pTAPS[j].GetDirection(),pTAPS[j].GetKinetic(),0.);
      fMggTAPS[fNggTAPS] = (g1 + g2).Mag();
      Double_t diff = TMath::Abs(fMggTAPS[fNggTAPS]-134.97666);
      if (diff <= min)
      {
	fMggBestTAPS = fMggTAPS[fNggTAPS];
	min = diff;
      }
      ++fNggTAPS;
      if (fNggTAPS == fNggMax) break;
    }
  }
  
  // >=1 pi0 in TAPS
  fHasPi0TAPS = fMggBestTAPS >= 80. && fMggBestTAPS <= 200.;
  fHasPi0TAPSPol10 = fHasPi0TAPS && fBeamPol10;
  fHasPi0TAPSPol20 = fHasPi0TAPS && fBeamPol20;
  
  // >=1 pi0 in CB or CB-TAPS or TAPS
  fHasPi0 = fHasPi0CB || fHasPi0CBTAPS || fHasPi0TAPS;
  fHasPi0Pol10 = fHasPi0CBPol10 || fHasPi0CBTAPSPol10 || fHasPi0TAPSPol10;
  fHasPi0Pol20 = fHasPi0CBPol20 || fHasPi0CBTAPSPol20 || fHasPi0TAPSPol20;
  
}

//_____________________________________________________________________________________
Bool_t TA2TestEGPhysics::TriggerProcessSW()
{
...
//_____________________________________________________________________________________
Bool_t TA2TestEGPhysics::BaF2Contrib(UInt_t element)
{
...
//_____________________________________________________________________________________
Bool_t TA2TestEGPhysics::TriggerProcessHW()
{
...
//_____________________________________________________________________________________
Bool_t TA2TestEGPhysics::HasCherCoinc(const Double_t &t)
{
...
//_____________________________________________________________________________________
Bool_t TA2TestEGPhysics::IsBgdCB(TA2Particle* p)
{
...
//_____________________________________________________________________________________
Bool_t  TA2TestEGPhysics::IsBgdTAPS(TA2Particle* p)
{
...
//_____________________________________________________________________________________
Bool_t TA2TestEGPhysics::IsPmtTrig(const Double_t &dT)
{
...
//_____________________________________________________________________________________
Bool_t TA2TestEGPhysics::IsRndTrig(const Double_t &dT)
{
...
//_____________________________________________________________________________________
Bool_t TA2TestEGPhysics::IsPmtCB(const Double_t &dT)
{
...
//_____________________________________________________________________________________
Bool_t TA2TestEGPhysics::IsRndCB(const Double_t &dT)
{
...
//_____________________________________________________________________________________
Bool_t TA2TestEGPhysics::IsPmtTAPS(const Double_t &dT)
{
  // Check if dT is inside of the CB prompt window
...
//_____________________________________________________________________________________
Bool_t TA2TestEGPhysics::IsRndTAPS(const Double_t &dT)
{
  // Check if dT is inside of at list one the CB random windows
...
void TA2TestEGPhysics::ResetEvent()
{
  ...
  // Pi0
  fNggCB = fNggCBTAPS = fNggTAPS = 0;
  fMggCB[0] = fMggCBTAPS[0] = fMggTAPS[0] = EBufferEnd;
  fMggBestCB = fMggBestCBTAPS = fMggBestTAPS = -1000.;
  fHasPi0CB = fHasPi0CBPol10 = fHasPi0CBPol20 = 0;
  fHasPi0CBTAPS = fHasPi0CBTAPSPol10 = fHasPi0CBTAPSPol20 = 0;
  fHasPi0TAPS = fHasPi0TAPSPol10 = fHasPi0TAPSPol20 = 0;
  fHasPi0 = fHasPi0Pol10 = fHasPi0Pol20 = 0;
...
//_____________________________________________________________________________________
void TA2TestEGPhysics::MarkEndBuffer()
{
  ...
  fMggTAPS[fNggTAPS] = EBufferEnd;
  
  // Charged
  fEpid[fNch] = EBufferEnd;
  fEcb[fNch]  = EBufferEnd;
